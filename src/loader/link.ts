...
class UrlLink extends Link {
    ...
    resolve(context: ColladaParsingContext) {
        ...
        var object: ColladaElement = context.ids[this.url];
        if (object != null) {
            this.target = object;
        } else {
...
class FxLink extends Link {
    url: string;
    scope: ColladaElement;

    constructor(url: string, scope: ColladaElement) {
        super();
        this.url = url;
        this.scope = scope;
    }
    ...
    resolve(context: ColladaParsingContext) {
        var scope: ColladaElement = this.scope;
        var object: ColladaElement = null;
    ...
    */
    static findSidTarget(url: string, root: ColladaElement, sids: string[], context: ColladaProcessingContext): ColladaElement {
        ...
        for (var i: number = 0, ilen: number = sids.length; i < ilen; i++) {
            ...
            var queue: ColladaElement[] = [parentObject];
            // Dept-first search
            while (queue.length !== 0) {
                // Get front of search queue
                var front: ColladaElement = queue.shift();
                ...
                var frontChildren: ColladaElement[] = front.sidChildren;
                if (frontChildren != null) {
                    for (var j: number = 0, jlen: number = frontChildren.length; j < jlen; j++) {
                        var sidChild: ColladaElement = frontChildren[j];
                        queue.push(sidChild);
                    }
                }
            }
